...
try {
  ...
  for (const path of ['booster/normalised-walk-ready/walk-forward/walk-forward-00', 'booster/normalised-walk-ready/walk-forward/walk-forward-01', 'anticipation/anticipation-00', 'jab/jab-00', 'recovery/recovery-00', 'block/block-00', 'recoil/recoil-00']) {
    assert.ok(drawn.some(src => src.includes(path)), `never drew ${path}`)
  }
  // Oracle's retreat: which of its four drawings a short scripted retreat shows depends
  // on the frame counter, so require it to have animated (two distinct drawings).
  const retreat = new Set(drawn.filter(src => src.includes('/oracle/normalised-walk-ready/walk-back/')))
  assert.ok(retreat.size >= 2, `oracle retreat drew ${retreat.size} distinct drawing(s)`)
  ...
  for (const path of ['/booster/normalised-walk-ready/walk-forward/', '/oracle/normalised-walk-ready/walk-back/', '/jab/', '/recovery/', '/block/', '/recoil/']) {
  ...
  assert.match(await missing.locator('#asset-status').innerText(), /60\/61 sprites ready; 1 failed — box fallback/)

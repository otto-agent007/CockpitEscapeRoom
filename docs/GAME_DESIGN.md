...
Both simulator scenarios play a synthesized engine ambience by default — a low rumble, a sawtooth hum whose harmonics rise with intensity, and a filtered noise bed, all generated at runtime with no audio asset. The topbar Sound button mutes it; if WebAudio is unavailable the button stays "Sound off" and flight is unaffected.

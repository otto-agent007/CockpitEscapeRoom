...
## 2026-08-28 — DC-9 Memphis neutral assembly (unapproved)

- RED: `python3 -m unittest tools.blender.cockpit_pipeline.tests.test_kmem_legacy_layout` failed with `ModuleNotFoundError` because `kmem_legacy_layout` had not yet been created. GREEN: the same command passed 2/2 after the exact five anchor and three Concourse-transform contract was added.
- Blender 5.1.2 ran `tools/blender/assemble_dc9_memphis_legacy.py` with `--background --factory-startup --disable-autoexec`; it verified Task 6 approval, source manifest, candidate GLB, and metadata hashes before importing only the approved candidate. It wrote the unapproved staged neutral blend/GLB, resolved layout, reimport report, 1440/768/375 PNGs, runtime contract, and computed-hash manifest.
- `python3 -m tools.blender.cockpit_pipeline.pipeline_cli validate-gate runtime-contract art-source/cockpit-pipeline/gates/dc9-memphis-legacy-runtime-contract.json` and `python3 -m tools.blender.cockpit_pipeline.pipeline_cli validate-manifest art-source/cockpit-pipeline/jobs/dc9-memphis-legacy-assembly/manifests/assembly-complete.json` both passed.
- `npx gltf-transform validate art-source/cockpit-pipeline/stages/assembly/output/dc9-memphis-legacy-assembly/dc9-memphis-legacy-neutral.glb` had no errors or warnings; 11 unused-UV rows were informational source-mesh notices. `git diff --check` passed.
- The PNG previews at `art-source/cockpit-pipeline/stages/assembly/output/dc9-memphis-legacy-assembly/previews/neutral-{1440,768,375}.png` were inspected only for readable nonblank output. Assembly Review Gate remains pending owner judgment of terminal framing/readability and the intentionally compressed, non-geographic 1995 memory composition. No browser integration or production/public-model promotion occurred.

## 2026-08-28 — DC-9 Memphis Assembly Review Gate approved

- Owner approved the revised 36 mm neutral framing for materials work. The approval record pins the current manifest, neutral blend/GLB, runtime contract, layout/report, and revised 1440/768/375 PNG hashes; it does not approve textures/final fidelity, browser/runtime integration, or public-model promotion.
- Fresh final handoff checks passed: `python3 -m unittest tools.blender.cockpit_pipeline.tests.test_kmem_legacy_layout` (2/2); runtime-contract and computed-hash manifest validation; `assembly_complete -> assembly-approved` transition validation; GLB validation with no errors/warnings (informational unused UV rows only); exact Blender reimport of all five anchors; source/assembly approval hash consistency; and `git diff --check`.
- Final reimport retained `KMEM_LEGACY_ROOT`, the five exact anchor coordinates, 15 meshes, 376 triangles, and 3 materials. Removed only the reproducible generated `dc9-memphis-legacy-neutral.blend1` backup; the approved neutral `.blend` remains.

## 2026-08-28 — DC-9 Memphis materials approval and production asset

- RED-first contract: `node --test tools/assets/dc9-memphis-model-contract.test.mjs` initially failed with `ERR_MODULE_NOT_FOUND`; after implementing the production validator it passed. The contract requires the ten stable nodes, exact five anchor `game_id` values, finite transforms, no cockpit-interaction metadata or excluded library names, and budgets of 5,000 triangles, six materials, 2048x1024 selected textures, and 8 MiB.
- Owner selected the original restrained grade after rejecting the brighter/warmer repair as too brown. Exact comparison hashes reproduced at 1440/768/375; the selected GLB reproduced SHA-256 `73b80e6f388b15c853b1ec39b6af6a31b36da040d447f7a6cc916ea7924d346b`.
- Fresh Blender 5.1.2 semantic validation passed for promoted master SHA-256 `4e8cc6cc6a7a3dcef71f1f4579efda4c2a17f49e2dee7ee62db9c818ed487d3d`: names/hierarchy/transforms, 36 mm camera, exact pre-warm lighting values, material/node assignments, packed texture identities/dimensions, 25 objects, 15 meshes, 376 triangles, 5 materials, anchors/game IDs/extras, immutable Task 7 inputs, and GLB reimport.
...
## 2026-08-28 — DC-9 Memphis terminal composition, canopy, and windshield browser proof

- **Assembly re-run (west frontage + ground repair + canopy).** RED-first layout tests grew to 14 (retired east transforms and retired ground set pinned as failing regressions), with the pure validator now measuring ramp-start windshield coverage from the measured first-officer rig (74.8% at 1440, 83.2% at 768, 97.3% at 375), a 15.2 m worst pavement shoulder, and ≥50.9 m route-corridor clearance. Agent 2 renders six windshield-pose previews beside the 36 mm reviews. A GLB read-back caught the glTF importer's QUATERNION rotation mode silently discarding `rotation_euler` writes (the previously shipped piers were never rotated); the script now forces XYZ and fail-closed verifies `matrix_world` and reimported transforms.
- **Shading and promotion.** Owner reviewed the canopy-bearing previews in session; approval-002 records pin current hashes. Agent 3 preserved the owner-ruled restrained grade, adding only `KMEM_CANOPY_MATERIAL` and the ramp material on `KMEM_TERMINAL_APRON` (27 objects / 17 meshes / 676 triangles / 6 materials). `npm run asset:dc9-memphis`, the three Memphis contract test files, `npm run assets:check`, and `npm run pipeline:evals` (6/6) passed. Deployable GLB SHA-256 `4732f1dcfff0f999ed77cb008b225b7d980be210553397f267554cd0c71045be`; loader version `4732f1dc`.
- **Far-plane defect, reproduced then repaired.** Every DC-9 camera is authored with `zfar = 100 m`, clipping the entire environment beyond 100 m — recorded in Discoveries before repair, with the refreshed headed captures as evidence. RED: a new `waitForMemphisEnvironment` assertion demanded a published `data-dc9-camera-frustum` far plane ≥ 1500 m and failed. GREEN: `memphisDeparture` alone widens the frustum to near 0.05 / far 2500 via `applyDc9GameplayCameraTransform`; all other stages keep authored planes, and the smoke camera-state contract is untouched.
- **Browser proof.** `e2e/dc9-memphis-departure.spec.ts`: 7/7 headless (evidence test opt-in). Frame budget headed on the GTX 1050 Ti with the widened frustum: median 16.7 ms, p95 16.7 ms (≤35 required), scene objects stable [28, 28, 28], no WebGL errors. Thirteen deterministic evidence PNGs captured headed at 375/768/1440 across ramp start, taxi turn, hold short, runway lineup, initial climb, reduced motion, and the skip3d panel — Concourse B and the canopy row read through the windshield at every width.
- **Stale journey coverage repaired.** The two smoke tests never re-verified after the Memphis insertion (the checkpoint run was killed before they reported): the staging test now proves the real Memphis request/camera/frustum after route submission before staging Home Operations from a completed departure, and the complete reordered journey plays the departure's native happy path end to end under `?skip3d=1` (120 s budget). Both pass in isolation (3.1 m and 36.3 s); ESLint, TypeScript, 561 Vitest tests, and the production build pass.

## 2026-08-28 — DC-9 Memphis ground clearance repairs the climb dash glitch

- Owner reported a beige plane over the flooring/thrust quadrant in the initial-climb captures: a runway centerline dash pitched up through the cabin because the aircraft reference sat at pavement level and the cockpit floor lay below the ground plane.
- RED: updated `dc9MemphisVisuals.test.ts` demanded an exported `DC9_MEMPHIS_GROUND_CLEARANCE_METERS` folded into the fixed-cockpit invariant plus a new plane-distance test at rest, full nose-up, and full bank (8 failed). GREEN: the world pose lifts the aircraft reference 2.5 m above the pavement; all 564 Vitest tests, ESLint, and TypeScript pass. `kmem_legacy_layout.SEAT_CAMERA` rose to eye z 3.20 (14 layout tests pass; windshield coverage unchanged at 74.8/83.2/97.3%).
- Headed re-capture of all thirteen evidence PNGs: climb views clean at 375/768/1440 and reduced motion; the ramp-start frontage still reads at every width. `e2e/dc9-memphis-departure.spec.ts` 7/7 headless; frame budget headed unchanged at median/p95 16.7 ms with scene objects stable [28, 28, 28] and no WebGL errors.

## 2026-08-29 — DC-9 Memphis retry, easier tuning, background scenery, and connected roofline

- **Always-on retry (owner request).** The departure panel's safe-retry control is now visible through every active beat as "Retry from checkpoint" (previously gated behind hint level 3 or a load failure); behavior is unchanged — it restores only the latest checkpoint and never erases earned beats. All four e2e references updated.
- **Gentler rules (owner request), RED-first.** Corridor warning 0.32→0.4, restore threshold 0.55→0.6 with the grace window 0.75 s→1.1 s, climb bands 0.3/0.28→0.45/0.4 with a decoupled 1.5 s instability window, early-rotation mistake split (0.48 strong pull) from the rotation requirement (0.25 gentle pull), and a wider hold-short approach. New Vitest proves gentle-roll tolerance and gentle-pull rotation; 565 unit tests pass. Two e2e drives were re-tuned to genuinely leave the softer corridor (longer wrong-steer holds, from a settled stop in the accessible test); the full Memphis spec then passed twice back-to-back (14/14 with --repeat-each=2).
- **Background scenery + connected roof (owner request).** `KMEM_FIELD` closes the void horizon under everything; west/north/east tree lines frame the frontage, runway end, and east apron (validated ≥100 m from the guided route by a new layout check); `KMEM_TERMINAL_CLERESTORY` — a dark recessed band reusing the runway matte — connects the martini-glass canopy to the terminal roof. Assembly/shading re-ran under approval-003 records at 32 objects / 22 meshes / 736 triangles / 8 materials (contract budget raised from six with rationale); all gates, manifests, GLB validation, 10 contract tests, `assets:check`, and pipeline evals pass. Deployable GLB SHA-256 `e730c591…`; loader version bumped.
- **Browser proof.** Memphis spec 7/7 (twice); frame budget headed unchanged at median/p95 16.7 ms with scene objects stable [33, 33, 33]; repaired smoke staging test passes headless (3.5 m) and the journey — now playing the departure with the new tuning — passes headed; the thirteen evidence PNGs were re-captured headed showing the retry button, field, tree lines, and seated canopy at every width. Known environment artifact: headed Chromium requests `/favicon.ico` (404, no icon link in `index.html`) which trips the staging test's zero-console-error assertion only in headed mode; the suite's supported headless run is unaffected — a one-line favicon link is a suggested follow-up.

## 2026-08-29 — DC-9 Memphis brake removal, readable takeoff, and two completion root causes

- **Owner direction.** Remove the never-used hold-brake control, make the takeoff better, and make completion go straight into the next part of the gameplay.
- **Root cause 1 (dev-only frozen panel, reproduced then repaired).** Under `npm run dev` React StrictMode's double-mount permanently disposed the HTML publication scheduler held in a `useRef`, freezing the whole departure panel (beat labels, guidance, 'Ready to line up', pitch buttons) while the 3D world kept moving — unplayable in dev, invisible to e2e because the suite tests the production preview build. Reproduced with a headed Playwright probe against the dev server (canvas beat reached `rotation` while the header still read 'Line up' and no 'Pull column aft' button existed). Fixed by lazy scheduler creation with the ref nulled on unmount; the same probe now shows the header tracking beats within ~100 ms.
- **Root cause 2 (completion dead-end, reproduced then repaired).** The taxi unsafe-crossing handed out the canonical hold-short frame while the durable checkpoint stayed `taxiTurn`; the strictly-adjacent reducer then silently rejected every later checkpoint and `COMPLETE_DC9_MEMPHIS_DEPARTURE` no-oped — a frozen 'complete' frame with no controls and no retry. Any player who crossed the hold without the brake hit it, which matches the owner never using the brake. Fixed: the crossing rewinds to the taxi checkpoint like every other mistake; locked by a Vitest regression (frame equals canonical `taxiTurn` and durable progress stays aligned) and a headed probe that makes the mistake, retries, and completes to Home Operations (attempts recorded, `checkpoint: 'complete'`).
- **Brakeless physics.** Fictional energy now chases the lever position (spool 0.32/s, coast 0.55/s, taxi capped 0.45, path rate 0.24) so closed levers always coast to a calm stop; near the hold a closed-lever approach pins at the painted boundary, a short stop idles gently to it (creep 0.16), and lever closing counts as stopping intent so an in-progress close never reads as a violation. Levers close at 1.8/s vs 0.9/s advance. `Hold brake` button, global Space binding, `normalizeDc9BrakeDemand`, `brakeHeld` runtime API, and `data-dc9-space-owner` plumbing all deleted; Space is never claimed by the DC-9 (`isDc9ControlKey` simplified, asserted by unit test).
- **Readable takeoff.** Staged roll guidance ('Rolling. Let the energy build…' → 'Speed is alive… rotation is coming' → 'Rotate — ease the column aft and hold it'), rotation as a held 0.5 s gentle pull with visible nose lift (`altitudeProgress` carries 0.15 through liftoff), an abandoned roll or unpulled rotation quietly returns to the lineup marker (no recorded mistake), climb shows an HTML 'Climbing out over Memphis' progress meter, and completion sets the status message 'Memphis legacy departure complete. The Home Operations Log is open.' Measured headed on the dev server: lineup→roll 0.9 s, roll→rotation cue 1.87 s, held rotation 0.59 s, climb ~1.8 s; full native flight ramp start→Home Operations 13.5 s. Beat ids, checkpoint ids, world anchors, cameras, and the GLB are untouched — no asset or layout change.
- **Unit tests.** The three departure test files were rewritten for the brakeless model (51 tests incl. new coverage: closed-lever settle, boundary coast, slow-coast-is-not-a-mistake, unsafe-crossing rewind regression, spool-to-roll, held-rotation, quiet runway reset, staged-guidance copy pinning the 'Home Operations Log is ready' completion promise); determinism/cadence invariance retained. Full Vitest: 568 tests / 44 files pass.
- **e2e.** `dc9-memphis-departure.spec.ts` rewritten for close-and-coast choreography and the held rotation (beat-label-driven waits replace the old 950 ms/250 ms taps); the brake pointer/Space test became 'the departure exposes no brake control and Space stays with the native hold buttons' (its first headed run exposed that a still-focused hold button keeps Space — the test now blurs before the body-focus assertion). Headed on hardware GL (GTX 1050 Ti): 11 passed, 1 env-gated evidence skip, including all three real-environment completion proofs and the skip3d every-native-beat run (Home Operations within its 8 s bound). `smoke.spec.ts` journey plays the brakeless departure end to end; full smoke headed: 33 passed, 3 triaged — control-check sweep and Airbus GLB failures were display contention from concurrent headed probes during that run (both pass in clean isolated headed reruns; journey re-passed alongside), and the DC-9 staging failure is the documented headed-only favicon-404 artifact. The Airbus test also passes headless (supported baseline); no Airbus file changed in this diff.
- **Browser paths exercised on the new build (headed dev-server probes + e2e).** Success (native buttons and keyboard-only), unsafe crossing → hint → retry → completion, hint ladder ×3, reduced-motion keyboard flight to Home Operations, mid-motion reload restoring the durable hold-short checkpoint with zeroed levers, blur/visibility pause, 375/768/1440 with no horizontal overflow (screenshots captured; no brake button at any width).
- **Docs/plan.** Plan 0040: dated Decision-log entry, two dated Discoveries (both root causes), Scope/file-structure/Validation updates; design spec amended 2026-08-29 (brake passages struck, fail-safe hold-short invariant retained). GAME_DESIGN/VISUAL_REALISM passages remain owed by Task 11, now to be written against this build.
- **Adversarial diff review — two further defects found and fixed.** (a) *Rotation lift discarded one frame after liftoff.* Every checkpoint commit snaps the live frame to `canonicalDc9DepartureFrame(checkpoint)`, so the new `altitudeProgress: 0.15` handed over at liftoff was immediately reverted to 0 — measured headed by sampling the climb meter across the hand-off (meter read `0` for the whole climb). Fixed by seeding canonical `initialClimb` with `ROTATION_LIFT_PROGRESS`, which also stops a mid-climb restore putting the aircraft back on its wheels; re-measured headed the meter now enters the climb at `15`. Pinned by a new Vitest case asserting the rotation hand-off frame equals the canonical climb frame on altitude and path progress (27 tests in that file). (b) *`skipOnSoftwareRenderer` was a check that could never fire.* It read `getContext('webgl2')` from the app's canvas, whose context three.js already owns, so it always returned `"unavailable"` and never matched the software pattern — the three real-environment completion tests and the frame-budget test therefore ran (and failed on drive-timing desync) in the supported headless suite rather than skipping. Fixed to probe a canvas of its own, `"unavailable"` now counts as ungated, and the frame-budget test (a hardware-rate budget by construction) got the same gate. Verified both directions: headless the four skip with the renderer-named reason (7 passed / 5 skipped), headed they execute and pass (11 passed / 1 env-gated skip) with `DC9_MEMPHIS_FRAME_METRICS median 16.7 ms / p95 16.7 ms, objects [33, 33, 33]`, renderer `ANGLE (NVIDIA GTX 1050 Ti, OpenGL 4.5.0)`.
- **Full suites.** `npm run check` (ESLint + tsc + Vitest + build) passes; Vitest 569 tests / 44 files. Full headless `npx playwright test`: 78 passed with the four hardware-rate tests now correctly skipped rather than failing. Full headed smoke: 33 passed, 3 triaged as environmental (two display-contention flakes that pass in clean isolated headed reruns, one documented headed-only favicon-404 artifact).

## 2026-08-29 — DC-9 Memphis ground motion: the taxi lurch the owner reported

- **Owner report.** "The ground on the first part of controlling the DC-9 is glitching, it's not smooth like when the plane is centered and doing to take off."
- **Reproduced and measured, two compounding causes.** Sampling the published world pose headed at a steady cadence with the levers latched: world speed climbed to ~95–120 m/s through `rampRelease`, then **collapsed to 1.4 m/s** at the `taxi` boundary and slowly rebuilt. A pure replica of the sampler showed the second half: at constant energy the speed *steps* 1014 → 407 m per unit progress (0.40×) at the 0.12 knot, and varies 2.4× *within* the ramp segment. The takeoff felt smooth because it contains no checkpoint commit and lies entirely inside one spline segment — exactly the contrast the owner described.
  1. *Checkpoint commits destroyed the aircraft's energy.* The effect keyed on `progress.checkpoint` re-seeded the live frame from `canonicalDc9DepartureFrame(...)` — a deliberately **stopped** pose — on every checkpoint, not just on entry/reload. Fixed by tracking when the running simulation produced the checkpoint itself and skipping the re-seed then; entry, reload, restore, and externally changed saves still seed exactly as before. Energy now also carries through rotation into the climb (climb-out handoff measured 1.79 s → 0.79 s).
  2. *The route was parameterized by knot fraction, not distance.* The legs are very unevenly long (105 m ramp, 136 m taxi, 35 m hold-to-lineup, 468 m runway) while their progress spans are 0.12/0.30/0.10/0.48, so metres-per-progress differed 2.8× between legs and stepped at every boundary. `dc9MemphisVisuals.ts` now measures the curve's arc length (96 samples per segment, cached per anchor map) and maps progress to distance with a monotone Fritsch–Carlson cubic through the anchor distances: the knots stay pinned exactly to their anchors while speed stays continuous across them. The geometry is untouched — same anchors, same Catmull-Rom curve, same point set — so all layout/coverage validation conclusions still hold.
- **Third defect found while probing: a single "Ready to line up" press could be silently swallowed.** The confirmation was consumed after one simulation frame and was rejected outright unless the *live* frame already read settled, while the button is drawn from a frame published up to 80 ms earlier. The e2e suite had been clicking it up to three times in a loop to get past this. The intent now latches until the aircraft actually settles and is withdrawn the moment the player commands thrust again; the rules still decide, so the fail-safe hold-short invariant is unchanged (the e2e assertion that a press during motion is rejected still passes).
- **Evidence.** New Vitest guard `travels at a continuous speed across every checkpoint knot` samples 200 points and caps the worst adjacent-sample speed step at 8%: the new sampler measures **6.5%**, the old parameterization measures **110%** — the check demonstrably discriminates. Anchors still land exactly at their knots. Vitest 570/570; `npm run check` passes. Headed re-measurement shows continuous speed through the ramp→taxi boundary (~59→80 m/s, no collapse). Departure e2e 11 passed headed (twice, before and after the lineup-latch fix), 7 passed / 5 correctly skipped headless; smoke journey and control-check sweep pass headed. Full-flight, mistake→retry→completion (now with a **single** lineup click), and keyboard-only probes all pass; windshield captures through the turn are clean.

## 2026-08-29 — DC-9 Memphis: the ground fight fixed by depth bias, and the airport outside from the chapter's first frame

- **Owner direction.** "The ground is still too glitchy during the taxi part, so we should just get rid of it. Also have the dc9 scene open up with the airport already outside, no need to transition into it." Presented with the measurement below, the owner chose to keep the pavement and fix the tie (depth bias) rather than delete the ground, and chose daylight through the whole chapter.
- **Measured cause, before changing anything.** The 2026-08-29 motion repairs did not fix the complaint because the artifact was never motion. Parsing the shipped GLB, `KMEM_RAMP`, `KMEM_TAXI_SURFACE` and `KMEM_RUNWAY_SURFACE` produce world top faces with float32 bits `0x00000000` — bit-identical — and overlap over 39.5 × 215 m (ramp/taxi) and 51 × 40 m (taxi/runway). Sampling the route, those patches are traversed at progress 0.1115→0.2660 and 0.5105→0.5530, i.e. the taxi legs; the runway leg overlaps nothing. A screen-area census through the authored windshield: 18–76 % of visible ground pixels through ramp release/taxi/hold short/lineup, **0.0 % at every takeoff, rotation and climb sample** — the whole reported asymmetry. Secondary: `KMEM_FIELD` is 8 cm under every pavement top and the frustum resolves `1.192e-6·z²` m, so 8 cm stops resolving past 259 m. Ruled out by arithmetic: shadow acne (no Memphis mesh sets `castShadow`/`receiveShadow`), the vibration term (gated to takeoff/rotation only — it would make the takeoff the bad phase), texture aliasing (all ground materials are untextured constant-colour BSDFs), double-sidedness, float32 transform error (worst at the climb, best at early taxi — the opposite of the report).
- **Fix.** `stageMemphisClone` assigns a depth-slope-scaled polygon offset per ground slab (field 0 < ramp/apron −1 < taxiway −2 < runway −3 < centreline −4), so surface ownership is decided by a fixed rule at every distance rather than by float luck. Materials are cloned per (source material, level), which keeps the bias off `KMEM_TERMINAL_CLERESTORY` — a building band that shares `KMEM_RUNWAY_MATERIAL` with the runway. Runtime-only: the GLB, the layout module, the pinned approval hashes and both owner asset gates are untouched. A vertical stagger was rejected (resolving two surfaces at 400 m needs 19 cm, a visible step); deleting the pavement was rejected once measured (it would re-create the 2026-08-28 route-over-void defect and break the `KMEM_RAMP`/`KMEM_TAXI_SURFACE`/`KMEM_RUNWAY_SURFACE` runtime node contract).
- **A/B browser proof, headed on the GTX 1050 Ti at 1440×900.** Five frames captured through a moving taxi with the bias on and off (same seed, same checkpoint, same route). The pavement is untextured flat colour, so a moving camera changes *nothing* inside a surface — any interior pixel that changes between two taxi frames has changed which surface owns it. Flat-ground flipped pixels over four consecutive frame pairs: **99,350 without the bias → 22,953 with it**; worst single pair 45,871 → 11,324; affected columns 635 → 188; median flip magnitude 21/255, matching the ramp-vs-taxi luminance step. The flip mask without the bias is textbook Moiré banding across the ground band; with the bias the only surviving region is the real taxiway edge sweeping past. The stills show it directly: the striped banding on the right-hand pavement of the taxi-turn frame is gone.
- **Airport outside from the chapter's first frame.** Five coupled changes, because the mount gate was only one of them: (1) `Dc9MemphisEnvironment` mounts for the whole `dc9` phase; (2) the wide frustum (near 0.05 / far 2500) now applies at every DC-9 stage — every DC-9 camera is authored with a 100 m far plane, so the mount alone would still have clipped the 700 m-deep environment away; (3) a `parked` prop holds the environment at the canonical ramp-start pose for every stage but the departure, without which the *completed* live frame (path 1, altitude 1) would leave the airport 110 m below and 700 m behind for Home Operations, the instrument scan, the shutdown and the key reveal; (4) the GLB is preloaded with the cockpit from the briefing screen and the DC-9 loading page holds while it is in flight, so the chapter cannot open on empty windows and have the airport fade up afterwards — a pop-in is itself the transition that was asked to be removed; (5) the dark `#070b0d` background and parked rig are gone, Memphis daylight lights the chapter, and `Dc9RuntimeLighting` is reduced to an interior fill (ambient 0.14→0.10→0.06, panel glow 0.34→0.10) that keeps the ceremonial shutdown's per-control dimming arc as a fade inside a daylit cockpit.
- **Tests updated to the new contract, not weakened.** The four `not.toHaveAttribute('data-dc9-memphis-model-state')` teardown assertions became `expectParkedMemphisEnvironment`, which is strictly stronger: it requires the environment to be staged *and* to be holding the parked pose, asserting the inverse world translation magnitude is < 10 m (the completed pose measures ≈ 719 m, so it fails by two orders of magnitude). The request-count test flipped from "zero requests before the route record" to "exactly one at chapter open and still exactly one after entering the departure", which now also proves no teardown-and-refetch across the stage change. `smoke.spec.ts` gained assertions at the route-record stage that the environment is ready, holding `rampRelease`, and carrying a far plane ≥ 1500 — the owner's requirement, checked in CI.
- **Results.** ESLint, TypeScript and 571 Vitest tests pass (new: ground slabs carry a strictly ordered bias, buildings sharing a ground material do not, and one clone per source-material/level). Memphis departure e2e headed: **11 passed**, 1 env-gated evidence skip, including all three real-environment completion proofs; frame budget with the environment mounted for the whole chapter and daylight running: **median 16.7 ms, p95 16.7 ms** (≤35 required), scene objects stable [33, 33, 33], no WebGL errors. DC-9 smoke test passes headless on SwiftShader (4.6 m) with the new opening assertions. The thirteen departure evidence PNGs were re-captured headed. `preview-renders/dc9-memphis-legacy-departure/chapter-daylight/` carries the daylit non-departure beats at all three widths; it was re-captured again after the 2026-08-30 swap, so its numbered filenames follow the current order.
- **Owner review still owed.** The daylight look across the eight DC-9 stages is a visual change behind approval gates (1) and (4) and has not been owner-reviewed. Note for that review: the control check (the chapter's opening stage) and the route/document stages use authored cameras that point down at the yoke and pedestal, so the airport reads through the upper window band rather than filling the view; re-aiming those opening cameras would be a separate decision.

## 2026-08-30 — DC-9 chapter: the Legacy Route Record and the Instrument Scan swap places

- **Owner direction.** "Let's switch the legacy route record with the instrument scan." Confirmed before implementing that this is a true swap — the completed instrument scan is what releases the Memphis departure, and the Legacy Route Record becomes the log written after the flight. New order: **control check → instrument scan → Memphis departure → Home Operations → Legacy Route Record → ceremonial shutdown → ATP → Captain's Key**.
- **Raised, not buried.** `plans/0032-dc9-right-seat-systems-check.md:126-129` records the *current* order as an explicit 2026-08-18 owner choice, whose rationale was that finishing the control-check sweep revealed the route strip on the same yoke. The swap reverses that decision and loses that adjacency; it is recorded as a superseding decision in plan 0040 rather than as a silent edit. CLAUDE.md's "Journey order — do not reorder or rename" was checked and is chapter-level only (opening → DC-9 → locker → Airbus → Tesla → Flight Mode → Mars); it names no DC-9 internal beat, and `plans/0032` set the precedent by inserting two DC-9 stages under the same rule.
- **The state machine.** Four transitions moved in `src/game/state.ts`: control-check completion → `instrumentScan`; scan completion → `memphisDeparture`; Home Operations completion → `intro`; correct route submission → `shutdown`. The route record occupies two stages — `intro` is the beat where the card sits unopened on the pedestal — and they travel together, because landing straight on `routeRecord` would open the dialog and skip the 3D card click entirely.
- **Copy that encoded the order was rewritten, not left to lie.** The control check no longer promises "a narrow paper strip clipped to the column"; the scan no longer says the aircraft "is ready to be put to bed" one beat before it flies; a new `routeIntro` line introduces the record after the flight; and the route submission now leads into the shutdown instruction. `hintFor` was scoped to the record's own stages — an unstamped record is now the normal state throughout the departure and Home Operations, so the old fallback would have answered a mid-takeoff hint request with route-puzzle advice.
- **Persistence was the load-bearing half.** The stage is re-derived from evidence on every load and never trusted from the save, so a naive swap fails *silently* — it teleports players rather than erroring, and no save-path test would have caught it. `GAME_SCHEMA_VERSION` is now 15; the bump is not bookkeeping but the only discriminator available, since a swap changes no stage names and an old-order and new-order save are otherwise byte-identical. `normalizeDc9Progress` is now a ladder of gaps — the first beat of the new order the player has not finished — with later evidence filling the earlier gates so no save is ever sent backwards. Completed routes imply the scan in both orders, which is what carries a schema-14 save across the swap; a save mid-flight lands mid-flight, a save mid-scan with everything else done lands at the shutdown. Route stamps are no longer zeroed by stage, because a migrated save can legitimately stand before a record it has already partly written.
- **A collision between two old guarantees, resolved deliberately.** A v12 save that had reached the shutdown used to be *offered* the newly inserted scan. Under the new order, offering it means marching a nearly finished chapter back to its opening beat. Such a save is now credited with the scan, exactly as it is already credited with the departure it predates — "never erase completed progress" beats "always offer new content". The changed expectation is written into `storage.test.ts` with that reasoning attached.
- **Tests re-sequenced to the new contract, not weakened.** 42 unit tests failed on the swap and every one was re-expressed rather than relaxed: `state.test.ts` gained `enterDc9InstrumentScan` / `enterDc9MemphisDeparture` / `completeDc9RouteRecord` helpers and now walks control check → scan → departure → Home Operations → record → shutdown; `storage.test.ts` migration tables carry explicit old-stage → new-stage mappings with the reasoning in comments. e2e: the smoke journey plays the new order end to end, the production-cockpit staging test now proves the gauge targets and the EPR reachability bound at the chapter's *opening* and the route-card projection *after* Home Operations, and the departure spec drives into the flight by answering the scan.
- **Results.** ESLint, TypeScript and **572 Vitest tests** pass. Memphis departure e2e headed on the GTX 1050 Ti: **11 passed**, 1 env-gated skip, including all three real-environment completion proofs; frame budget median 16.7 ms / p95 16.8 ms against the 35 ms limit, scene objects stable [33, 33, 33]. Smoke headless: the re-sequenced journey, accessible flow, control-check sweep, instrument scan, route record and the 6.4-minute production-cockpit staging test all pass.
- **Owner review still owed.** The swap changes the chapter's shape and sits behind approval gates (1) and (4), alongside the daylight change from 2026-08-29 that is also unreviewed. Worth the owner's eye at that gate: the `intro` beat renders no panel of its own, so after the Home Operations Log closes the player sees a bare cockpit with only the status line and the projected route card — that was acceptable when it opened the chapter and may want a prompt now that it closes it.

## 2026-08-30 — PR #68 browser-smoke timeout repair

- GitHub Actions run `33329476189` / browser-smoke job `99305347823` selected 85 Chromium cases: 77 passed, 7 skipped, and only `real Memphis environment is fetched once for the whole chapter and stays in the right seat` failed. All three attempts resolved the requested native scan button as visible, enabled and stable, then exhausted the 90-second test timeout while performing the fourth-to-sixth click.
- Root cause: the new whole-chapter environment test repeated all six instrument answers while rendering the 926k-triangle cockpit and real Memphis environment under SwiftShader. The unchanged focused test passed locally with retries disabled but took 1.1 minutes, which left too little variance inside its 90-second CI budget; prior measured evidence puts the same software renderer near 800 ms per frame versus 16.7 ms on the workstation GPU. No runtime failure or missing selector was reproduced.
- Test-only repair: the real-GLB fixture now starts with five durable scan answers and performs the real final EPR click across `instrumentScan → memphisDeparture`. Its owning contract remains intact: one Memphis request, no model-state teardown/remount, parked pose before the click, live environment/right-seat camera after it, no Model Y request or copy, and clean console. The independent lightweight smoke test still plays all six answers, including wrong-answer coaching and retained progress, so player-flow coverage was not removed.
- Focused GREEN: the real-GLB case passed in 28.2 seconds with retries disabled, then passed in 30.0 seconds beside the independent full six-answer smoke case (2/2 in 40.0 seconds). Independent review proved the remount and refetch mutations failed but found `parked={true}` still survived because the live departure begins at the parked pose; one bounded native thrust hold now requires a material world-pose change. That strengthened case passed in 34.6 seconds and killed the parked-only mutation. Final full Memphis headless spec: 7 passed, 5 documented hardware-rate skips, 0 failed in 2.0 minutes; the lifecycle case took 34.8 seconds inside that run.
- Repository verification: `npm run check` passed ESLint, TypeScript, 572/572 Vitest tests, and the Vite production build; `npm run assets:check` passed with only the existing informational glTF rows. Browser plugin was unavailable, so repository Playwright was used as required by the frontend-testing workflow.

## 2026-09-05 — DC-9 Memphis: the guided route and the hold are finally painted

- Owner report: "There is not curved path rendered or marked hold spot." Reproduced from the shipped asset before any change: the `dc9-memphis-legacy-departure.glb` JSON chunk holds 32 nodes and only nine painted nodes, all runway centreline dashes (`KMEM_CENTERLINE_01..09` at X −120, Y 280–664); nothing marks the ramp lead-out, the taxi curve or the hold-short boundary, while the panel copy reads "Follow the curved path toward the marked hold" and the design spec promised a highlighted lead-out path and a marked safe zone. The committed `1440-taxi-turn.png` and `1440-hold-short.png` showed bare pavement.
- Repair (runtime only, `plans/0042-dc9-memphis-route-markings.md`): `src/scenes/dc9MemphisRouteMarkings.ts` derives a 1.2 m faded-yellow guidance line from ramp release to the lineup knot and a hold-short marking (two solid bars, two dashed bars across 30 m, two 1.6 m posts beyond the ends) from the five GLB anchors through the same `sampleDc9MemphisPath` that moves the world, so the paint is the line `lateralError` is scored against. `stageMemphisClone` attaches the meshes under `KMEM_LEGACY_ROOT` at the shipped centreline's depth-bias level and disposes them with the clone. The GLB, Blender master, layout module and approval records are byte-identical to `origin/main` (`git diff --stat origin/main -- public/models art-source tools/blender` is empty).
- Two facts that shaped the geometry: the hold→lineup Catmull-Rom segment dips to −8.15 m because its trailing control point is the 110 m climb anchor (masked at runtime by `altitudeProgress`, fatal for paint, so ground paint pins z = 0.03), and through the centre panes at 1440 the pavement is visible only from ~40 m ahead (horizon y ≈ 132, glareshield y ≈ 195), so the hold marking reads on the approach and the posts carry the far read.
- Vitest: `dc9MemphisRouteMarkings.test.ts` written RED first (module missing), then 5/5; `Dc9MemphisEnvironment.test.ts` two new RED cases, then 7/7; scene suite 105/105; whole suite 579/579 (was 572). ESLint and `tsc -b` clean.
- Browser measurement, headed Chromium on `DISPLAY=:0`, this build served from port 4321 and an `origin/main` worktree build from port 4322, faded-yellow pixel census inside per-width windshield windows (1440: x 560–1440 / y 100–360; 768: x 0–768 / y 100–255; 375: x 0–375 / y 60–230): this build reads 608 / 339 / 271 hits at ramp start, a 189 / 151 / 136-pixel widest row at the taxi turn (the bars lying across the route ahead), 1727 / 721 / 425 hits with 472 / 352 / 245-pixel widest rows on the approach ~45 m short of the hold, 2175 hits stopped at the hold at 1440 (the bar beside the cockpit in the right window) and 0 hits lined up (the guidance ends at the lineup point; the beige runway dashes do not read as taxi yellow). The `origin/main` build reads 0 in every window at every view.
- New Playwright checks in `e2e/dc9-memphis-departure.spec.ts`, decoding the browser's own screenshot with no image library: `paints the guided route and the hold-short marking where the panel copy points` (all three widths at rest, thresholds at roughly half the measured values, plus the stopped-at-hold and lined-up bounds at 1440) and `the hold-short marking grows in the windshield on the approach` (hardware-rate only; keyboard drive). Against this build: 2 passed in 20.0 s. Against the `origin/main` build: both failed with `Received: 0`. The evidence capture now also records `{375,768,1440}-hold-short-approach.png`.

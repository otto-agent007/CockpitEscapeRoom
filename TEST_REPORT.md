...
| `npm run check` | Lint, typecheck, tests, and build pass | Blocked: initial run failed because `eslint` was missing; `npm install` became idle; `npm ci` hit repeated internal package gateway `ETIMEDOUT` tarball fetches and was stopped after more than seven minutes | Environment-blocked | Rerun after dependency installation succeeds |

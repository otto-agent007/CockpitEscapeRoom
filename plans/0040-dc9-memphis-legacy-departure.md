...
- brake input and an explicit safe lineup confirmation; *(2026-08-29: the brake was retired on owner direction — thrust-lever-only stopping via rolling friction, see Decision log)*
...
- 'src/game/useDc9MemphisDeparture.ts' — rAF loop, visibility pause, checkpoint dispatch, and published HTML frame (brake input retired 2026-08-29).
- 'src/components/dc9/Dc9AxisControls.tsx' — shared axis meters and native hold controls extracted from Control Check.
- 'src/components/dc9/MemphisDeparturePanel.tsx' — active beat, qualitative guidance, lineup, restore, climb progress, and accessibility UI (brake button retired 2026-08-29).
...
- **2026-08-29 — Dev-only frozen departure panel: a strict-mode remount permanently disposed the HTML publication scheduler (reproduced, repaired).** On the dev server (React `StrictMode` double-invokes effects) the departure's HTML panel froze at its mount-time frame forever: the unmount cleanup called `dispose()` on the scheduler held in a `useRef`, and the strict-mode remount kept that dead scheduler, so every later `request()` silently no-opped. The 3D world kept advancing (it reads `frameRef` per rAF) while the beat label, guidance, 'Ready to line up', and the pitch hold buttons never appeared — the departure was effectively unplayable in dev, invisible to e2e because the suite tests the production preview build, where strict-mode double-invocation does not happen. Reproduced with a headed Playwright probe against `npm run dev` (canvas `data-dc9-memphis-beat` reached `rotation` while the header still read 'Line up' and no 'Pull column aft' button existed). Fix: the scheduler is created lazily and the ref is released to `null` on unmount so a remount rebuilds a live one; proven by re-running the same probe (header tracks 'Memory lane' within 100 ms of the beat change). This most likely underlies both prior owner reports (the departure feeling broken, and completion "not returning to the rest of the DC-9 parts" when played in dev).

- **2026-08-29 — Real completion dead-end: the taxi unsafe-crossing handed out the hold-short frame while the durable checkpoint stayed at taxiTurn (reproduced, repaired).** The old taxi branch answered an open-lever hold crossing with `canonicalDc9DepartureFrame('holdShort')` plus a mistake event. The frame then played on from the hold, but the reducer accepts only strictly-adjacent checkpoint advances, so the durable save stayed at `taxiTurn`, silently rejected the later `runwayLineup` and `initialClimb` checkpoints, and `COMPLETE_DC9_MEMPHIS_DEPARTURE` (which requires durable `initialClimb`) no-opped — the flight ended on a frozen 'complete' frame with no controls and no retry. Any player who crossed the hold without the brake hit this, which matches the owner never using the brake. Repaired by rewinding the crossing to the taxi checkpoint like every other mistake (frame and durable state stay aligned); regression-locked by a Vitest case asserting the crossing frame equals canonical `taxiTurn` and by a headed probe that makes the mistake, retries, and completes to Home Operations.

- **2026-08-29 — `skipOnSoftwareRenderer` was a check that could never fire (found by adversarial diff review, repaired).** The renderer gate added with the completion proofs read `getContext('webgl2') ?? getContext('webgl')` from the *app's* canvas — three.js already owns that context, so the call returns null, the helper always answered `"unavailable"`, and the pattern never matched. The three real-environment completion tests and the frame-budget test therefore never skipped: in the supported headless suite they ran under SwiftShader and failed on the documented drive-timing desync (four failures in the 2026-08-29 full run). Repaired by probing a canvas of the helper's own, treating `"unavailable"` as ungated, and applying the same gate to the frame-budget test, whose 35 ms p95 budget is a hardware-rate claim by construction. Verified in both directions (headless 7 passed / 5 skipped with the renderer-named reason; headed 11 passed with the real NVIDIA renderer logged) — the check can now actually fail, which is what makes the headed evidence meaningful.

- **2026-08-29 — Owner report "the ground on the first part is glitching, not smooth like the takeoff" (reproduced, measured, repaired).** Two compounding causes, both confirmed headed before changing anything. (1) *Checkpoint commits zeroed the aircraft's energy*: the effect keyed on `progress.checkpoint` re-seeded the live frame from the deliberately **stopped** canonical pose on every checkpoint, not just on entry/reload — measured world speed collapsed from ~95–120 m/s to **1.4 m/s** at the ramp→taxi boundary and slowly rebuilt. Now the hook records when the running simulation produced the checkpoint itself and skips the re-seed for that one; entry, reload, restore, and externally changed saves seed exactly as before, and energy carries through rotation into the climb (climb handoff 1.79 s → 0.79 s). (2) *The route was parameterized by knot fraction, not distance*: legs of 105/136/35/468 m occupy progress spans of 0.12/0.30/0.10/0.48, so metres-per-progress differed 2.8× between legs and stepped 0.40× at the 0.12 knot while varying 2.4× within the ramp leg. `dc9MemphisVisuals.ts` now measures arc length (96 samples/segment, cached per anchor map) and maps progress to distance through a monotone Fritsch–Carlson cubic on the anchor distances — knots stay pinned exactly to their anchors, speed stays continuous across them, and **the curve geometry is unchanged** (same anchors, same Catmull-Rom, same point set), so every layout coverage/clearance conclusion still holds and no asset work is implied. The takeoff always felt smooth because it holds no checkpoint commit and lies inside one spline segment. Regression-locked by a Vitest guard capping the worst adjacent-sample speed step at 8% (new sampler 6.5%, old parameterization 110%).

- **2026-08-29 — A single "Ready to line up" press could be silently swallowed (found while probing the above, repaired).** The confirmation was consumed after one simulation frame and rejected unless the *live* frame already read settled, while the button is drawn from a frame published up to 80 ms earlier — which is why the e2e drives had been clicking it up to three times in a loop. The intent now latches until the aircraft settles and is withdrawn as soon as the player commands thrust again. The rules still arbitrate, so the spec's fail-safe hold-short invariant (stopped **and** explicitly confirmed before runway entry) is unchanged and its e2e assertion still passes.

- **2026-08-29 (evening) — The residual "glitchy ground" was depth fighting, not motion; measured, then proved with an A/B browser capture.** The 2026-08-29 motion repairs above did not fix the owner's complaint because the artifact was never motion. Three ground slabs are exactly coplanar: reading `public/models/dc9-memphis-legacy-departure.glb`, `KMEM_RAMP` (node.translation.y −0.75 + local +0.75), `KMEM_TAXI_SURFACE` (−0.699999988079071 + 0.699999988079071) and `KMEM_RUNWAY_SURFACE` (−0.7200000286102295 + 0.7200000286102295) all produce a world top face with float32 bits `0x00000000`. Overlaps: ramp×taxi X[−90,−50.5] Y[15,230] = 39.5 × 215 m; taxi×runway X[−140,−89] Y[225,265] = 51 × 40 m. Sampling the module's own route, the first is traversed at progress 0.1115→0.2660 and the second at 0.5105→0.5530 — i.e. the taxi legs — while the runway leg (0.55→1.0) crosses neither. A screen-area census through the authored windshield put the exactly-coplanar overlap at **18–76 % of visible ground pixels through ramp release / taxi / hold short / lineup and 0.0 % at every takeoff, rotation and climb sample**. Secondary: `KMEM_FIELD` sits 8 cm under every pavement top and the departure frustum (near 0.05 / far 2500, 24-bit) resolves only `1.192e-6·z²` m, so 8 cm stops resolving past **259 m** — present in both phases, so it aggravates rather than explains. Ruled out by arithmetic: shadow acne (no Memphis mesh ever sets `castShadow`/`receiveShadow`), the vibration term (gated to `takeoffRoll`/`rotation` only — if causal the takeoff would be the bad phase), texture aliasing (every ground material is an untextured constant-colour Principled BSDF), double-sidedness, and float32 transform error (largest at the climb, smallest during early taxi — the opposite of the report).
  **Browser A/B, headed on the GTX 1050 Ti at 1440×900**, capturing five frames through a moving taxi with the bias on and off. Because the pavement is untextured flat colour, a moving camera changes *nothing* inside a surface, so any interior pixel that changes between two taxi frames has changed which surface owns it. Flat-ground flipped pixels over four consecutive frame pairs: **99,350 without the bias, 22,953 with it**; worst single pair 45,871 → 11,324; affected columns 635 → 188. The flip mask without the bias is textbook Moiré banding across the ground; with the bias the only surviving region is the real taxiway edge sweeping past, and the median flip magnitude (21/255) matches the ramp-vs-taxi luminance step. The stills show the same thing directly: the striped banding on the right-hand pavement in the taxi-turn frame is gone.

- **2026-08-30 PR #68 browser-smoke timeout — test workload, not product behavior.** GitHub Actions run `33329476189`, job `99305347823`, passed 77 tests and failed only `real Memphis environment is fetched once for the whole chapter and stays in the right seat`; all three attempts found the requested scan button visible, enabled and stable, then exhausted the 90-second test budget while clicking the fourth-to-sixth gauge under SwiftShader. The unchanged focused case passed locally with retries disabled but took 1.1 minutes, confirming a narrow timing margin against the already measured ~800 ms software-renderer frame interval. The lifecycle contract needs only the final scan answer to cross `instrumentScan → memphisDeparture`; the independent lightweight smoke case already exercises all six answers, wrong-answer coaching and retained progress. Seeding five durable answers kept the real final click plus every request-count, MutationObserver, parked-pose, camera, console and spoiler assertion, and reduced the real-GLB case to 28.2–30.0 seconds. Independent review then caught that `parked={true}` still survived because the live frame begins at the canonical parked pose; one bounded native thrust hold now requires a material world-pose change and kills that mutation. The strengthened case passed in 34.6 seconds and the complete headless Memphis spec passed 7 with 5 hardware-rate skips in 2.0 minutes; `npm run check` passed ESLint, TypeScript, 572 Vitest tests and the production build.

## Decision log

- **2026-09-05 — The guided route is now painted (see `plans/0042-dc9-memphis-route-markings.md`).** The owner reported "There is not curved path rendered or marked hold spot": the shipped GLB carries only the nine runway dashes, so the panel's "highlighted path" and "marked hold" copy pointed at bare pavement. The taxi guidance line and the hold-short marking are derived at runtime from the five anchors by the same sampler that moves the world (`src/scenes/dc9MemphisRouteMarkings.ts`, attached in `stageMemphisClone`); the GLB, the Blender master, the layout module and every approval record are untouched. Follow-up still open from the spec: the amber → white → green colour states of the guidance line.


- **2026-08-30 — Real-asset lifecycle smoke starts at the last scan answer.** The real Memphis GLB case is responsible for proving one fetch, no teardown/remount across the stage boundary, the parked-to-live handoff, right-seat camera, console health and spoiler safety. It no longer repeats the first five scan answers under software rendering; `e2e/smoke.spec.ts` remains the independent full six-answer player-flow authority. This preserves the mutation that must fail (unmounting or reloading Memphis on `instrumentScan → memphisDeparture`) while removing work unrelated to that contract.

- **2026-08-30 — The Legacy Route Record and the Instrument Scan swap places (owner direction).** The owner asked to "switch the legacy route record with the instrument scan". Confirmed reading before implementing: a true swap, in which **the completed scan is what releases the Memphis departure** and the route record becomes the log written after the flight. New order: **controlCheck → instrumentScan → memphisDeparture → homeOperations → intro → routeRecord → shutdown → qualification → keyReveal**. The `intro` stage travels with the record — it is the beat where the card sits on the pedestal unopened, and landing straight on `routeRecord` would pop the dialog and skip the 3D card click.
  *What this supersedes.* The design spec's approved progression line (amended there), and the 2026-08-18 owner choice at `plans/0032-dc9-right-seat-systems-check.md:126-129`, whose stated rationale was that finishing the control-check sweep revealed the route strip on the same yoke. That adjacency is genuinely lost and was raised with the owner rather than quietly dropped. CLAUDE.md's "Journey order — do not reorder or rename" is chapter-level (opening → DC-9 → locker → Airbus → Tesla → Flight Mode → Mars) and names no DC-9 internal beat, so it is not in tension with this; `plans/0032` set the precedent by inserting two DC-9 stages under the same rule.
  *Four transitions moved,* all in `src/game/state.ts`: control-check completion → `instrumentScan`; scan completion → `memphisDeparture`; Home Operations completion → `intro`; route submission → `shutdown`. Copy that encoded the old order was rewritten rather than left to lie — the control check no longer promises "a narrow paper strip", the scan no longer says the aircraft "is ready to be put to bed" one beat before it flies, and a new `routeIntro` line introduces the record after the flight. `hintFor` was scoped: an unstamped record is now the normal state through the departure and Home Operations, and the fallback would otherwise have answered a mid-takeoff hint with route-puzzle advice.
  *Persistence is the load-bearing half.* The stage is re-derived from evidence on load, never trusted from the save, so a naive swap would have silently teleported players rather than erroring. `GAME_SCHEMA_VERSION` is 15 — the bump is not conventional bookkeeping but the only way to tell an old-order save from a new-order one, since a swap changes no stage names. `normalizeDc9Progress` now reads as a ladder of gaps — the first beat of the new order the player has not finished — with later evidence filling the earlier gates so nobody is ever sent backwards. Completed routes imply the scan in both orders, which is what carries a schema-14 save across the swap. Route stamps are no longer zeroed by stage, because a migrated save can legitimately stand before a record it has already partly written, and zeroing them there would erase earned progress.
  *Two old guarantees collided and one had to give.* A v12 save that had reached the shutdown used to be *offered* the newly inserted scan; under the new order offering it means marching a nearly finished chapter back to its opening. Such a save is now credited with the scan, exactly as it is already credited with the departure it predates — the "never erase completed progress" rule wins over "always offer new content". The changed expectation is recorded in `storage.test.ts` with that reasoning, not silently flipped.


- **2026-08-29 (evening) — The airport is outside the windows for the whole DC-9 chapter, with no transition into it (owner direction).** The owner asked that the DC-9 scene "open up with the airport already outside, no need to transition into it". This **supersedes** the 2026-08-27 'Separate environment GLB' decision's *lazy-load during `memphisDeparture`* clause, the Task 4 mount-only-at-departure step, acceptance line 'the environment mounts only during the departure', and the staging in which Memphis arrives with the flight. Everything else in those decisions stands: it is still a separate GLB, still a cockpit-first fixed right seat, still a 1995 memory recreation that returns to the parked Final Flight Log. Five coupled changes, because the mount gate was only one of them:
  1. `Dc9MemphisEnvironment` mounts for the whole `dc9` phase (`PrototypeScene.tsx` `CaptainCockpit`), so no stage change tears it down and refetches it.
  2. The wide frustum (near 0.05 / far 2500) applies at **every** DC-9 stage. Every DC-9 camera is authored with a 100 m far plane, so the mount alone would have shown nothing — the 700 m-deep environment was being clipped away.
  3. A new `parked` prop holds the environment at `canonicalDc9DepartureFrame('rampStart')` for every stage but the departure. Without it the *completed* live frame (pathProgress 1, altitudeProgress 1) would have left the airport 110 m below and 700 m behind for Home Operations, the instrument scan, the ceremonial shutdown and the key reveal. The parked pose is solved once per stage rather than every frame.
  4. The GLB is preloaded with the cockpit from the briefing screen and the DC-9 loading page holds while it is in flight, so the chapter cannot open on empty windows and have the airport fade up a beat later — a pop-in is itself the transition the owner asked to remove. A `loading` status always resolves to ready or error, so the page can be deferred but never stranded.
  5. Lighting: the dark `#070b0d` background and the parked-cockpit rig are gone. Memphis daylight lights the whole chapter (owner choice: "daylight through the chapter"), and `Dc9RuntimeLighting` is reduced to an interior fill — ambient 0.14→0.10→0.06 and a 0.34→0.10 panel glow — that keeps the ceremonial shutdown's per-control dimming arc as a fade inside a daylit cockpit rather than a slide into darkness.
  Cost accepted knowingly: the 1.92 MB environment is now part of the chapter's opening download (the cockpit it rides with is 36 MiB) and the daylight rig runs for every DC-9 stage. Measured headed after the change: p95 frame interval **16.7 ms** against the 35 ms budget, scene object count stable at [33, 33, 33].

- **2026-08-29 (evening) — The glitchy taxi ground is fixed with a depth bias; the ground stays (owner direction, reversing "just get rid of it").** The owner reported the ground still glitching through the taxi and asked to remove it. Measurement first: `KMEM_RAMP`, `KMEM_TAXI_SURFACE` and `KMEM_RUNWAY_SURFACE` have **bit-identical** top faces in the shipped GLB (world Y exactly `0x00000000`) with overlapping footprints — 8,492 m² ramp-under-taxiway and 2,040 m² taxiway-under-runway — and **both patches sit under the taxi legs while the runway leg north of Y 265 overlaps nothing**, which is the entire "glitchy on taxi, smooth on takeoff" asymmetry. Presented with that, the owner chose to keep the pavement and fix the tie. `stageMemphisClone` now assigns a depth-slope-scaled polygon offset per ground slab — field 0 < ramp/apron −1 < taxiway −2 < runway −3 < centreline −4 — so ownership is decided by a fixed rule at every distance. A vertical stagger was rejected: resolving two surfaces 400 m out needs 19 cm of separation under this frustum, which reads as a visible step, and a zero gap fights at any precision. Deleting the pavement was rejected once measured, because it would have re-created the 2026-08-28 route-over-void defect and broken the `KMEM_RAMP`/`KMEM_TAXI_SURFACE`/`KMEM_RUNWAY_SURFACE` runtime node contract. Runtime-only: the GLB, the layout module, the approval hashes and both owner asset gates are untouched. Materials are cloned per (source, level) so the bias cannot leak to `KMEM_TERMINAL_CLERESTORY`, which shares `KMEM_RUNWAY_MATERIAL` with the runway.

- **2026-08-29 — Brake retired; thrust-lever-only departure with a readable takeoff (owner direction).** The owner observed the hold-brake control was never used and asked to remove it, improve the takeoff, and guarantee completion flows straight into the next beat. This supersedes the design spec's 'separate fictional brake demand' (spec Controls section) while keeping its fail-safe hold-short invariant: the scene still cannot enter the runway until the aircraft is stopped in the safe zone and 'Ready to line up' is pressed — only the stopping mechanism changed. New model: fictional energy chases the lever position (spool 0.32/s, coast 0.55/s, taxi capped 0.45) so closed levers always coast to a calm stop; near the hold, a closed-lever approach pins at the painted boundary and a short stop idles gently forward to it (creep 0.16); an open-lever crossing rewinds to the taxi checkpoint. Levers close twice as fast as they advance (1.8/s vs 0.9/s) so 'close the levers' reads immediately. Takeoff: staged guidance ('Rolling — let the energy build' → 'Speed is alive… rotation is coming' → 'Rotate — ease the column aft and hold it'), a ~2 s building roll, rotation as a held 0.5 s gentle pull with visible nose lift (altitudeProgress carries 0.15 through liftoff), an abandoned roll quietly returns to the lineup marker, and the climb shows an HTML 'Climbing out over Memphis' progress meter. Completion dispatches an explicit status message ('Memphis legacy departure complete. The Home Operations Log is open.') and the restore path re-arms the completion event. Space is no longer claimed by the departure; the 'Hold brake' button, its Space binding, and `normalizeDc9BrakeDemand` are deleted. Beat ids, checkpoint ids, world anchors, cameras, and the GLB are untouched.
...
- **2026-08-27 — Separate environment GLB.** Do not rebuild or enlarge 'dc9-cockpit.glb'; lazy-load a KMEM-only model during 'memphisDeparture'. *(2026-08-29: the load window widened to the whole DC-9 chapter on owner direction — the separate-GLB rule itself is unchanged; see Decision log.)*
...
- no Space claim at any time (the brake and its Space binding are retired; stopping is the closed-lever coast with the taxi creep to the hold);
...
- [x] 2026-08-30 — PR #68 browser-smoke timeout reproduced from GitHub evidence and repaired without changing runtime behavior or weakening the lifecycle contract.
...
PR #68 browser-smoke repair evidence:

- RED: GitHub run `33329476189` / job `99305347823`: 77 passed, 7 skipped, 1 failed after three 90-second attempts in the whole-chapter real Memphis test; each failure stalled during the redundant six-answer scan drive.
- Baseline reproduction: unchanged focused test passed locally with retries disabled in 1.1 minutes, leaving little CI variance inside its 90-second limit.
- GREEN: minimized real-GLB transition case passed in 28.2 seconds, then 30.0 seconds beside the independent full-scan case; both tests passed. After review added the live world-pose mutation guard, the focused case passed in 34.6 seconds.
- Mutation review: forced remount produced `['ready', 'loading', 'ready']`, forced cache-clear/refetch produced two requests, and forced `parked={true}` failed the new world-pose assertion; all temporary mutations were restored.
- Nearby regression: the final full `e2e/dc9-memphis-departure.spec.ts` passed 7 tests with 5 documented hardware-rate skips in 2.0 minutes; the strengthened lifecycle case took 34.8 seconds inside that run.
- Repository gates: `npm run check` passed ESLint, TypeScript, 572/572 Vitest tests, and the Vite production build; `npm run assets:check` passed with only the existing informational glTF rows.

...
## 2026-07-16 Airbus radio and thrust target alignment follow-up

- Goal: move the projected Airbus radio drop area farther left on the radio panel and move the thrust drop area farther right to the midpoint between the paired thrust levers, preserving the pivot/hitbox/cue contract.
- Source coordinates: `AIRBUS_A320_TARGET_RADIO_PIVOT = (-0.040000, -0.474842, 0.011798)` and `AIRBUS_A320_TARGET_THRUST_PIVOT = (0.015000, -0.505764, 0.004800)` in Blender space. Exported glTF translations are `[-0.04, 0.011798, 0.474842]` and `[0.015, 0.0048, 0.505764]` after the `(x, z, -y)` axis conversion.
- TDD red evidence: `npm run assets:check` reported the old radio `-0.03` and thrust `0.003` exports; the focused browser test reported the old radio projection at approximately x=`934px`.
- Green evidence: `npm run assets:check` passed; `npx playwright test e2e/smoke.spec.ts --grep "Airbus production cockpit loads" --workers=1` passed 1/1 against the rebuilt GLB. The test proves projected ranges and real canvas raycaster placement for RADIO and THRUST, with a bounded 180-second teardown budget for SwiftShader.
- Blender/export evidence: Blender 5.1.2 source preparation and scene validation passed with the existing 124 imported-source warnings. The supported Airbus asset build completed the visual GLB export; a metadata-only rerun stalled in the environment's approval-render/EGL stage, so the final metadata-bearing GLB was regenerated with the supported `tools/blender/export_glb.py` exporter and passed `npx gltf-transform validate` plus `npm run assets:check`.
- Final deployable Airbus GLB: 39,878,776 bytes, SHA-256 `e340dcf1caefb998f208a5fd228455384d289916efd4b4f15fbafc50c79497ef`.
- Actual-browser evidence inspected at 1440x900, 768x900, and 375x900. Accepted 1440 proof: `preview-renders/airbus-target-alignment/airbus-radio-thrust-aligned-1440.png`; 768 remains usable. The 375 view retains the existing narrow projected-camera limitation with the thrust marker outside the viewport; mobile redesign was not part of this desktop Airbus alignment scope.
